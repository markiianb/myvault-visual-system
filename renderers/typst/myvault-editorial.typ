...
// ============================================================
// stat-infographic — embed a pre-rendered stat-infographic SVG into an ebook
//
// The SVG itself is generated by `renderers/vega-lite/compose.mjs` from a
// stat-infographic-{2,3,4}up fixture. This primitive wraps it at the right
// page measure. Per chunks/stat-infographic.md R-STAT-001..005, the infographic
// already carries its own signal-go border + divider + footer band.
//
// Sizing convention:
//   measure: "1-col" (640pt — fits inside the body column)
//             "wide"  (720pt — earns the breath; default for full-page-impact embeds)
//             "full"  (no scaling — uses the SVG's native width)
// ============================================================

#let stat-infographic(
  path,
  measure: "wide",
) = block(
  breakable: false,
  width: 100%,
  {
    let target-width = if measure == "1-col" { 640pt }
                        else if measure == "full" { auto }
                        else { 720pt }
...
// ============================================================
// diagram — embed a pre-rendered flow / system / pipeline diagram SVG
//
// Generated by `renderers/vega-lite/compose.mjs` from a flow-diagram fixture.
// Per chunks/diagram.md R-DIAG-001..006, the diagram already carries its
// own footer-at-top + body envelope. This primitive only sizes the embed.
//
// Sizing convention:
//   measure: "1-col" (640pt — fits inside body column; tight)
//             "wide"  (720pt — earns the breath; default for flow diagrams)
//             "full"  (no scaling — uses the SVG's native width)
// ============================================================
